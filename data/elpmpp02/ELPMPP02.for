...
!
!-----------------------------------------------------------------------
!     COMPUTE LUNAR COORDINATES (POSITIONS AND VELOCITIES):
!     21 DATES FROM JD 2444239.5, INTERVAL 500 DAYS (LLR).
!-----------------------------------------------------------------------
!
      tj0=2444239.5d0
      dtj=500.d0
...
!-----------------------------------------------------------------------
!     COMPUTE LUNAR COORDINATES (POSITIONS AND VELOCITIES):
!     15 DATES FROM JD 2500000.5, INTERVAL -200000 DAYS (DE405).
!-----------------------------------------------------------------------
!
      tj0=2500000.5d0
      dtj=-200000.d0
...
!
      implicit double precision (a-h,o-z),integer(i-n)
!
      dimension       t(0:4),xyz(6),v(6),vmain(3)
!
      parameter       (max1=2645,max2=33256)
      parameter       (cpi=3.141592653589793d0)
...
!
      enddo
! --- DEBUG: main-only and full (main+pert) series sums for tj=JD2444239.5
      if (dabs(tj+7305.5d0).lt.0.1d0) then
        write(*,*) ' DEBUG tj=',tj,' v_main(1)=',vmain(1),
     &    ' v_main(2)=',vmain(2),' v_main(3)=',vmain(3)
        write(*,*) ' DEBUG full v(1)=',v(1),' v(2)=',v(2),' v(3)=',v(3)
      endif
! --- DEBUG for JD 2500000.5 (tj=48455.5, DE405): full v(1)v(2)v(3) [arcsec,arcsec,ELP]
      if (dabs(tj-48455.5d0).lt.0.1d0) then
        write(*,*) ' DEBUG JD2500000.5 full v(1)=',v(1),' v(2)=',v(2),
     &    ' v(3)=',v(3)
      endif
!
...
!
      if (dabs(tj+7305.5d0).lt.0.1d0) then
        write(*,*) ' DEBUG after W1: v1_rad=',v(1),' v2_rad=',v(2),
     &    ' v3_km=',v(3)
      endif
! --- DEBUG JD2500000.5: after W1 (lon_rad, lat_rad, r_km)
      if (dabs(tj-48455.5d0).lt.0.1d0) then
        write(*,*) ' DEBUG JD2500000.5 after W1: v1_rad=',v(1),
     &    ' v2_rad=',v(2),' v3_km=',v(3)
      endif
!
      clamb  = cos(v(1))
      slamb  = sin(v(1))
...
!
      ...
      x3     = sw
      if (dabs(tj+7305.5d0).lt.0.1d0) then
        write(*,*) ' DEBUG x1=',x1,' x2=',x2,' x3=',x3
      endif
! --- DEBUG JD2500000.5: x1,x2,x3
      if (dabs(tj-48455.5d0).lt.0.1d0) then
        write(*,*) ' DEBUG JD2500000.5 x1=',x1,' x2=',x2,' x3=',x3
      endif
...
! --- DEBUG JD2500000.5: X,Y,Z (final position km)
      if (dabs(tj-48455.5d0).lt.0.1d0) then
        write(*,*) ' DEBUG JD2500000.5 X=',xyz(1),' Y=',xyz(2),' Z=',xyz(3)
      endif
!
