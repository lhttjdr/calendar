...
export const quaternion= (...args) =>{
  if(args.length===1){ // contract
    ...
    if(q.hasOwnProperty("scalar") && q.hasOwnProperty("vector")){
      return {scalar:decimal(q.scalar), vector:vector(q.vector)}; // construct
    }
  }else if(args.length===2){
    let s=decimal(args[0]), v=vector(args[1]);
    if(Vector.dimension(v)!==3) throw new TypeError("Except a 3d vector!");
...
export const grossman = std.uncurry(p=> q=> {
  ...
  return quaternion(
    ...
    Vector.plus(Vector.plus(Vector.mult(q.vector, p.scalar), Vector.mult(p.vector, q.scalar)), Vector.cross(p.vector, q.vector))
  );
});
