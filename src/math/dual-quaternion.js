...
export const dualquaternion= (...args) =>{
  if(args.length===1){
    ...
    if(q.hasOwnProperty("real") && q.hasOwnProperty("dual")){
      return {real:quaternion(q.real), dual:quaternion(q.dual)}; // construct
    }
  }else if( args.length===2){
    let r=quaternion(args[0]), d=quaternion(args[1]);
    return {real: r, dual:d};
  }
  throw new TypeError("Except a DualQuaternion!");
}
export const plus=std.uncurry(p=> q=>{
  p=dualquaternion(p), q=dualquaternion(q);
  return dualquaternion(Quaternion.plus(p.real, q.real), Quaternion.plus(p.dual, q.dual));
});
export const mult=std.uncurry(p=> q=>{
  p=dualquaternion(p), q=dualquaternion(q);
  return dualquaternion(
    Quaternion.grossman(p.real, q.real),
    Quaternion.plus(Quaternion.grossman(p.real,q.dual), Quaternion.grossman(p.dual, q.real))
  );
});
export const scale= std.uncurry(q=> r=>{
   q=dualquaternion(q), r=decimal(r);
...
export const normalize= q=>{
  ...
  return dualquaternion(
    Quaternion.mult(q.real,inv),
    Quaternion.minus(Quaternion.mult(q.dual,inv),Quaternion.mult(q.real, Decimal.mult(Decimal.cube(inv),Quaternion.dot(q.real, q.dual))))
  );
}

export const eq= std.uncurry(a=> b=>{
  a=dualquaternion(a), b=dualquaternion(b);
