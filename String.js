(function() {
  var _String = Calendar.createNS("Calendar.String");
  /**
   *
   *  Javascript sprintf
    ...
    init: function() {
      ...
      for (i = 0; i < matches.length; i++) {
        if (matches[i].code == '%') {
          ...
          matches[i].argument = matches[i].argument.substring(0, matches[
              i].precision ? matches[i].precision : matches[i].argument
            .length)
          substitution = sprintfWrapper.convert(matches[i], true);
  ...
  _String.sprintf = sprintfWrapper.init;
})();
